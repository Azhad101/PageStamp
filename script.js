...
(function () {
  ...
  const Joy = (function () {
    ...
    function burst(originEl, opts) {
      ...
      for (let i = 0; i < options.count; i++) {
        ...
        const py = Math.sin(angle) * dist - Math.random() * 60;
    ...
    function toast(message, icon) {
      ...
      el.className = 'toast';
      el.innerHTML = `<span class="toastIcon">${icon || '✦'}</span><span>${message}</span>`;
  ...
  const Sound = (function () {
    ...
    function toggleMuted() {
      muted = !muted;
      try { localStorage.setItem(STORAGE_KEY_SOUND, muted ? 'muted' : 'on'); } catch (e) { }
      return muted;
    }
  ...
  const Officer = (function () {
    ...
    function renderToast(message, icon) {
      const el = document.createElement('div');
      el.className = 'officerToast';
      el.innerHTML = `
        <div class="officerBadge">${icon || '🛂'}</div>
        <div class="officerBody">
          <div class="officerHead">PASSPORT OFFICER</div>
          <div class="officerMsg">${message}</div>
        </div>
      `;
      document.body.appendChild(el);
      requestAnimationFrame(() => el.classList.add('toastShow'));
  ...
  const DOM = {
    ...
    navItems: document.querySelectorAll('.navMenu .navItem'),
    views: document.querySelectorAll('.view'),
    
    sidebarAvatar: document.getElementById('sidebar-avatar'),
    sidebarHolderName: document.getElementById('sidebar-holder-name'),
    
    passportSpread: document.querySelector('.passportSpread'),
    passportAgingLayer: document.getElementById('passport-aging-layer'),
    ...
    stampFilterBtns: document.querySelectorAll('.stFilters .filterBtn'),
    ...
    genreChips: document.querySelectorAll('.chipBtn'),
    ...
    emojiBtns: document.querySelectorAll('.emojiBtn'),
  ...
  function playBootAnimation() {
    ...
    if (DOM.appLayout) DOM.appLayout.classList.remove('revealed');

    DOM.bootOverlay.classList.remove('hidden');
    DOM.bootOverlay.style.pointerEvents = '';
    DOM.bookFrontCover.classList.remove('open');

    const triggerOpen = () => {
      DOM.bookFrontCover.classList.add('open');
      Sound.pageFlip();
      ...
      setTimeout(() => {
        ...
        const revealSite = () => {
          if (DOM.appLayout) DOM.appLayout.classList.add('revealed');
        };
  ...
  function initSidebarHover() {
    ...
    const openSidebar = () => {
      ...
      DOM.sidebar.classList.add('sideOpen');
    };

    const closeSidebar = () => {
      hoverTimeout = setTimeout(() => {
        DOM.sidebar.classList.remove('sideOpen');
      }, 350);
    };
    ...
    if (DOM.btnToggleSidebar) {
      DOM.btnToggleSidebar.addEventListener('click', () => {
        DOM.sidebar.classList.toggle('sideOpen');
      });
    }
  }
  ...
  const AGING_LEVELS = [
    { min: 0, stamps: 0, visas: 0 },
    { min: 5, stamps: 2, visas: 1 },
    { min: 15, stamps: 4, visas: 2 },
    { min: 35, stamps: 6, visas: 3 },
    { min: 65, stamps: 8, visas: 4 },
    { min: 100, stamps: 11, visas: 6 }
  ];

  const STAMP_COUNTRY_CODES = ['LEC', 'BKW', 'NVL', 'FIC', 'PGE', 'TLE', 'RDR', 'STK'];
  const VISA_LABELS = [
    { label: 'ENTRY', code: 'RL' }, { label: 'TRANSIT', code: 'BK' },
    { label: 'VISA', code: 'PG' }, { label: 'ARRIVAL', code: 'NV' },
    { label: 'READER', code: 'FC' }, { label: 'EXPEDITION', code: 'TL' }
  ];

  function renderAgingDecorations(level) {
    if (!DOM.passportAgingLayer) return;
    const layer = DOM.passportAgingLayer;
    layer.innerHTML = '';
    if (level <= 0) return;

    const config = AGING_LEVELS[level];

    for (let i = 0; i < config.stamps; i++) {
      const rand = (n) => seededRandom(i * 97 + n * 13 + level);
      const size = 46 + Math.round(rand(1) * 26);
      const top = 8 + rand(2) * 76;
      const left = 4 + rand(3) * 88;
      const rot = -35 + rand(4) * 70;
      const el = document.createElement('div');
      el.className = 'agingStamp';
      el.style.width = size + 'px';
      el.style.height = size + 'px';
      el.style.top = top + '%';
      el.style.left = left + '%';
      el.style.setProperty('--age-rot', rot.toFixed(1) + 'deg');
      el.style.setProperty('--age-op', (0.35 + level * 0.05).toFixed(2));
      el.style.animationDelay = (i * 0.05) + 's';
      const code = STAMP_COUNTRY_CODES[i % STAMP_COUNTRY_CODES.length];
      el.innerHTML = `<span class="agingTop">${code}</span><span class="agingMid">★</span>`;
      layer.appendChild(el);
    }

    for (let i = 0; i < config.visas; i++) {
      const rand = (n) => seededRandom(i * 61 + n * 19 + level + 500);
      const w = 48 + Math.round(rand(1) * 20);
      const h = 32 + Math.round(rand(2) * 16);
      const top = 6 + rand(3) * 80;
      const left = 6 + rand(4) * 84;
      const rot = -18 + rand(5) * 36;
      const el = document.createElement('div');
      el.className = 'agingVisa';
      el.style.width = w + 'px';
      el.style.height = h + 'px';
      el.style.top = top + '%';
      el.style.left = left + '%';
      el.style.setProperty('--age-rot', rot.toFixed(1) + 'deg');
      el.style.setProperty('--age-op', (0.4 + level * 0.04).toFixed(2));
      el.style.animationDelay = (i * 0.06 + 0.1) + 's';
      const visa = VISA_LABELS[i % VISA_LABELS.length];
      el.innerHTML = `<span class="agingLabel">${visa.label}</span><span class="agingCode">${visa.code}</span>`;
      layer.appendChild(el);
    }
  }
  ...
  function applyPassportAging(finishedCount) {
    ...
    DOM.passportSpread.classList.remove('wear-1', 'wear-2', 'wear-3', 'wear-4', 'wear-5');
    let level = 0;
    for (let i = AGING_LEVELS.length - 1; i >= 1; i--) {
      if (finishedCount >= AGING_LEVELS[i].min) { level = i; break; }
    }
    if (level > 0) DOM.passportSpread.classList.add(`wear-${level}`);
    renderAgingDecorations(level);
  }
  ...
  function renderPassportFirstPage() {
    ...
    if (currentlyReadingBook) {
      DOM.homeCurrentBookContainer.innerHTML = `
        <div class="curCard">
          <img src="${escapeHtml(currentlyReadingBook.coverUrl)}" alt="Cover" class="miniCover" onerror="this.onerror=null; this.src='https://via.placeholder.com/40x58/2b1a1d/d4af37?text=Book';">
          <div class="curDetails">
            <h4 class="curTitle">${escapeHtml(currentlyReadingBook.title)}</h4>
            <p class="curAuthor">${escapeHtml(currentlyReadingBook.author)}</p>
            <div class="miniBar">
              <div class="miniFill" style="width: ${currentlyReadingBook.progress}%;"></div>
            </div>
          </div>
          <button class="btn btnSecondary btnSm" onclick="window.PageStamp.navigateTo('reading')">Continue</button>
        </div>
      `;
    } else {
      DOM.homeCurrentBookContainer.innerHTML = `
        <div style="text-align: center; color: var(--text-muted); padding: 0.5rem;">
          ...
          <button class="btn btnSecondary btnSm" style="margin-top: 0.35rem;" onclick="window.PageStamp.navigateTo('search')">Find a Book</button>
        </div>
      `;
    }
  ...
  function renderStampCollage(allFinished) {
    ...
    if (filtered.length === 0) {
      DOM.passportCollageContainer.innerHTML = `
        <div class="emptyCard" style="grid-column: 1 / -1; background: transparent; border: 1px dashed var(--border-paper); padding: 2rem 1rem;">
          ...
          <button class="btn btnPrimary" style="margin-top: 0.75rem;" onclick="window.PageStamp.navigateTo('search')">Search Books</button>
        </div>
      `;
    ...
    DOM.passportCollageContainer.innerHTML = pageItems.map((book, idx) => {
      ...
      let moodBadgeClass = 'moodLoved';
      let moodIcon = '❤️';
      if (book.moodTag === 'It was okay') { moodBadgeClass = 'moodOk'; moodIcon = '📖'; }
      if (book.moodTag === 'Not for me') { moodBadgeClass = 'moodBad'; moodIcon = '💔'; }

      return `
        <div class="stampCard" style="transform: rotate(${angle}deg);" onclick="window.PageStamp.openStampDetail('${book.id}')">
          <div class="stHead">
            <span class="stCode">STAMP #${globalIdx + 1}</span>
            <span class="stMood ${moodBadgeClass}">${moodIcon} ${escapeHtml(book.moodTag || 'Finished')}</span>
          </div>

          <div class="stBody">
            <img src="${escapeHtml(book.coverUrl)}" alt="Cover" class="stThumb" onerror="this.onerror=null; this.src='https://via.placeholder.com/42x60/2b1a1d/d4af37?text=Book';">
            <div class="stMeta">
              <h4 class="stTitle">${escapeHtml(book.title)}</h4>
              <p class="stAuthor">${escapeHtml(book.author)}</p>
              <p class="stDate">VERIFIED: ${escapeHtml(book.dateFinished || '2026-07-24')}</p>
            </div>
          </div>
  ...
  function renderStampPageControls(totalPages) {
    const pageNum = 1 + state.activeStampPage;
    ...
    for (let p = 1; p <= totalPages; p++) {
      dotsHtml += `<button type="button" class="dot ${p === state.activeStampPage ? 'active' : ''}" data-page="${p}" title="Page ${p + 1}" onclick="window.PageStamp.goToStampPage(${p})"></button>`;
    }
  ...
  function renderSearchResults(docs) {
    DOM.searchResultsGrid.innerHTML = docs.map(doc => {
      ...
      const coverHTML = coverUrl 
        ? `<img src="${escapeHtml(coverUrl)}" alt="${escapeHtml(title)}" class="coverImg" onerror="this.outerHTML='<div class=\\'placeholder-cover\\'><span class=\\'placeholder-icon\\'>📖</span><span class=\\'placeholder-title\\'>${escapeHtml(title)}</span></div>'">`
        : `<div class="phCover"><span class="phIcon">📖</span><span class="phTitle">${escapeHtml(title)}</span></div>`;

      return `
        <div class="bookCard passCard">
          <div class="passStrip">
          ...
          <div class="bookInfo">
            <h4 class="bookTitle">${escapeHtml(title)}</h4>
            <p class="bookAuthor">${escapeHtml(author)}</p>
            <div class="bookActions">
              <button class="btn btnSecondary btnSm" onclick="window.PageStamp.addBook('${escapeHtml(title)}', '${escapeHtml(author)}', '${coverUrl || ''}', 'want-to-read')">
                🧳 Pack for Trip
              </button>
              <button class="btn btnPrimary btnSm" onclick="window.PageStamp.addBook('${escapeHtml(title)}', '${escapeHtml(author)}', '${coverUrl || ''}', 'currently-reading')">
                ✈️ Board Now
              </button>
            </div>
          </div>
  ...
  function renderShelf() {
    ...
    if (wantBooks.length === 0) {
      DOM.shelfBooksGrid.innerHTML = `
        <div class="emptyCard">
          ...
          <button class="btn btnPrimary" style="margin-top: 1rem;" onclick="window.PageStamp.navigateTo('search')">Search Books</button>
        </div>
      `;
      return;
    }
    ...
    DOM.shelfBooksGrid.innerHTML = wantBooks.map(book => `
      <div class="bookCard" draggable="true" data-book-id="${book.id}">
        <div class="tagFlap">🏷️ DESTINATION</div>
        <div class="coverWrap">
          <img src="${escapeHtml(book.coverUrl)}" alt="${escapeHtml(book.title)}" class="coverImg" onerror="this.outerHTML='<div class=\\'placeholder-cover\\'><span class=\\'placeholder-icon\\'>📖</span><span class=\\'placeholder-title\\'>${escapeHtml(book.title)}</span></div>'">
        </div>
        <div class="bookInfo">
          <h4 class="bookTitle">${escapeHtml(book.title)}</h4>
          <p class="bookAuthor">${escapeHtml(book.author)}</p>
          <div class="bookActions">
            <button class="btn btnPrimary btnSm btnBlock" onclick="window.PageStamp.startReadingFromShelf('${book.id}')">
              ✈️ Board Flight
            </button>
            <button class="btn btnOutline btnSm btnBlock" onclick="window.PageStamp.deleteBook('${book.id}')">
              Unpack
            </button>
          </div>
        </div>
  ...
  function renderCurrentlyReading() {
      ...
      DOM.readingActiveContainer.innerHTML = `
        <div class="emptyCard">
          ...
          <div style="display: flex; gap: 1rem; justify-content: center; margin-top: 1.25rem;">
            <button class="btn btnSecondary" onclick="window.PageStamp.navigateTo('shelf')">View Suitcase</button>
            <button class="btn btnPrimary" onclick="window.PageStamp.navigateTo('search')">Search New Books</button>
          </div>
        </div>
      `;
      return;
    ...
    DOM.readingActiveContainer.innerHTML = `
      <div class="showCard" draggable="true" data-book-id="${activeBook.id}">
        <div class="ribbon"></div>
        <div class="showCoverWrap">
          <img src="${escapeHtml(activeBook.coverUrl)}" alt="${escapeHtml(activeBook.title)}" class="showCover" onerror="this.outerHTML='<div class=\\'placeholder-cover\\'><span class=\\'placeholder-icon\\'>📖</span><span class=\\'placeholder-title\\'>${escapeHtml(activeBook.title)}</span></div>'">
        </div>
        <div class="showDetails">
          <h3 class="showTitle">${escapeHtml(activeBook.title)}</h3>
          <p class="showAuthor">by ${escapeHtml(activeBook.author)}</p>

          <div class="progBox">
            <div class="progHead">
              <span class="progLabel">FLIGHT PROGRESS</span>
              <span class="pctBadge" id="reading-percent-num">${activeBook.progress}%</span>
            </div>

            <div class="flyTrack">
              <span class="flyNode flyStart">🧳</span>
              <div class="flyLine">
                <div class="flyFill" id="flight-route-fill" style="width:${activeBook.progress}%"></div>
                <span class="flyIcon" id="flight-plane-icon" style="left:${activeBook.progress}%">✈️</span>
              </div>
              <span class="flyNode flyEnd">🏁</span>
            </div>
            
            <div class="sliderBox">
              <button class="btnStep" id="btn-step-minus" title="Decrease 5%">-</button>
              <input type="range" min="0" max="100" value="${activeBook.progress}" class="slider" id="reading-slider-input">
              <button class="btnStep" id="btn-step-plus" title="Increase 5%">+</button>
            </div>
          </div>

          <div style="display: flex; gap: 1rem; align-items: center;">
            <button class="btn btnGold btnBlock" style="padding: 0.9rem;" onclick="window.PageStamp.openFinishModal('${activeBook.id}')">
              ✦ Land the Flight & Stamp Passport ✦
            </button>
          </div>

          <div class="padZone" id="stamp-pad-dropzone">
    ...
    const showcaseCard = document.querySelector('.showCard[draggable="true"]');
    ...
    if (stampPad) {
      stampPad.addEventListener('dragover', (e) => {
        ...
        stampPad.classList.add('dragOver');
      });
      stampPad.addEventListener('dragleave', () => {
        stampPad.classList.remove('dragOver');
      });
  ...
  function openFinishModal(bookId) {
    ...
    DOM.animatedStampBadge.classList.remove('stampDown');
    DOM.modalFinish.classList.remove('hidden');

    const paper = document.querySelector('.stCanvas');
  ...
  function handleFinishFormSubmit(e) {
    ...
    DOM.animatedStampBadge.classList.add('stampDown');
    ...
    if (paper) {
      setTimeout(() => {
        ...
        ring.className = 'bleedRing bleedOn';
    ...
    setTimeout(() => {
      ...
      const burstOrigin = document.querySelector('.passportSpread') || document.body;
  ...
  function openStampDetail(bookId) {
    ...
    DOM.detailModalContent.innerHTML = `
      <div class="dtBody">
        <div class="dtHead">
          <img src="${escapeHtml(book.coverUrl)}" alt="Cover" class="dtCover" onerror="this.onerror=null; this.src='https://via.placeholder.com/100x150/2b1a1d/d4af37?text=Book';">
          <div>
            <h3 class="dtTitle">${escapeHtml(book.title)}</h3>
            <p class="dtAuthor">by ${escapeHtml(book.author)}</p>
        ...
        <div style="display: flex; justify-content: flex-end; margin-top: 1.5rem; gap: 0.75rem;">
          <button class="btn btnOutline btnSm" onclick="window.PageStamp.deleteBook('${book.id}')">Delete Stamp</button>
        </div>
      </div>
    `;
  ...
  function renderChallengeVisaPage() {
    ...
    for (let i = 0; i < goal; i++) {
      ...
      if (finishedBook) {
        slotsHTML += `
          <div class="visaSlot filled">
            <div class="visaMini">
              <span class="visaIcon">🏵️</span>
              <span class="visaMiniTitle">${escapeHtml(finishedBook.title)}</span>
              <span class="slotNum">${finishedBook.dateFinished || 'VERIFIED'}</span>
            </div>
          </div>
        `;
      } else {
  ...
  function renderSouvenirGrid() {
    ...
    DOM.souvenirGrid.innerHTML = Souvenirs.CATALOG.map(item => {
      ...
      return `
        <div class="souvCard ${isCollected ? 'collected' : 'locked'}" title="${isCollected ? escapeHtml(item.blurb) : 'Keep reading to unlock this souvenir'}">
          <span class="souvIcon">${isCollected ? item.icon : '❔'}</span>
          <span class="souvTitle">${isCollected ? escapeHtml(item.title) : '???'}</span>
        </div>
      `;
    }).join('');
  }
  ...
  function showNextSouvenirReveal() {
    ...
    DOM.souvenirModalContent.innerHTML = `
      <div class="souvReveal">
        <div class="officerLine">🛂 THE PASSPORT OFFICER SLIDES SOMETHING ACROSS THE DESK</div>
        <div class="pcCard">
          <span class="pcIcon">${item.icon}</span>
          <h3>${escapeHtml(item.title)}</h3>
          <p>${escapeHtml(item.blurb)}</p>
        </div>
        <button type="button" class="btn btnGold" id="btn-souvenir-continue">Add to Collection</button>
      </div>
    `;
  ...
  function toggleMilestone(element, isUnlocked) {
    ...
    if (isUnlocked) {
      ...
      element.querySelector('.badgeStatus').textContent = 'UNLOCKED! ✦';

      if (!wasUnlocked) {
        element.classList.add('justUnlocked');
        setTimeout(() => element.classList.remove('justUnlocked'), 750);
        Joy.burst(element, { count: 18, spread: 140, duration: 0.9 });
        Sound.chime();
        const label = element.querySelector('.badgeTitle');
        Officer.say('milestone', { label: label ? label.textContent : 'Reading Visa' }, '🏵️');
      }
    } else {
      element.classList.remove('unlocked');
      element.querySelector('.badgeStatus').textContent = 'Locked';
    }
  }
  ...
  function renderWorldAtlas() {
    ...
    if (DOM.atlasPinsLayer) {
      DOM.atlasPinsLayer.innerHTML = COUNTRY_ATLAS.map(country => {
        ...
        return `
          <g class="pinWrap" data-country="${country.code}" transform="translate(${country.x}, ${country.y})">
            <title>${escapeHtml(titleText)}</title>
            <g class="pin ${isUnlocked ? 'unlocked' : ''}">
              <path class="pinDrop" d="M0,-16 C7,-16 12,-11 12,-4 C12,4 0,16 0,16 C0,16 -12,4 -12,-4 C-12,-11 -7,-16 0,-16 Z"></path>
              <text class="pinIcon" y="-2">${isUnlocked ? country.flag : '❔'}</text>
              ${isUnlocked && bookCount > 1 ? `<circle cx="9" cy="-14" r="7" fill="var(--gold2)"></circle><text x="9" y="-11" text-anchor="middle" font-size="8" font-weight="700" fill="#2b1a1d">${bookCount}</text>` : ''}
            </g>
          </g>
        `;
      }).join('');
    ...
    if (DOM.atlasLegend) {
      ...
      let legendHtml = unlockedCodes.map(code => {
        ...
        return `
          <div class="legendCard" data-country="${code}">
            <span class="legendFlag">${entry.country.flag}</span>
            <div>
              <div class="legendCountry">${escapeHtml(entry.country.name)}</div>
              <div class="legendBooks">${bookList}</div>
            </div>
          </div>
        `;
      }).join('');
      ...
      if (unmappedBooks.length > 0) {
        ...
        legendHtml += `
          <div class="legendCard legendMissing" data-country="unmapped" title="We don't have origin data for this author yet">
            <span class="legendFlag">❔</span>
            <div>
              <div class="legendCountry">Unmapped Authors</div>
              <div class="legendBooks">${unmappedList}</div>
            </div>
          </div>
        `;
      }
  ...
  function initEventListeners() {
    ...
    if (DOM.atlasPinsLayer) {
      DOM.atlasPinsLayer.addEventListener('click', (e) => {
        const pin = e.target.closest('.atlas-pin-wrap');
        if (!pin) return;
        const code = pin.getAttribute('data-country');
        const card = DOM.atlasLegend ? DOM.atlasLegend.querySelector(`.legendCard[data-country="${code}"]`) : null;
        if (card) {
          card.scrollIntoView({ behavior: 'smooth', block: 'center' });
          card.classList.add('justUnlocked');
          setTimeout(() => card.classList.remove('justUnlocked'), 750);
        } else {
    ...
    if (DOM.boardingGateDropzone) {
      DOM.boardingGateDropzone.addEventListener('dragover', (e) => {
        e.preventDefault();
        e.dataTransfer.dropEffect = 'move';
        DOM.boardingGateDropzone.classList.add('dragOver');
      });
      DOM.boardingGateDropzone.addEventListener('dragleave', () => {
        DOM.boardingGateDropzone.classList.remove('dragOver');
      });
